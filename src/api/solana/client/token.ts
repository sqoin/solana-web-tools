...
import { ADAPTABLE_PROGRAMM_ID, ASSOCIATED_TOKEN_PROGRAM_ID, MINT_ADDRESS, ORIGINE_PROGRAMM_ID, PORTFOLIO_MINT_ADDRESS, PORTFOLIO_PROGRAMM_ID, SENTINEL_ACCOUNT_ADDRESS } from '../../../pages/WalletUtils';
import { TOKEN_PROGRAM_ID } from '@solana/spl-token';
import { CurveType, TokenSwap } from './token-swap';
import { nToken } from './nToken';
...
//@ts-ignore
export const PortfolioAccountLayout: typeof BufferLayout.Structure = BufferLayout.struct(
  [
    Layout.publicKey('owner'), //  32
    Layout.publicKey('metadataURL'), //128
    Layout.publicKey('metadataHASH'), // 2
...
export class Token {
  ...
  async createAccount(owner: PublicKey): Promise<PublicKey> {
    ...
    console.log("*****token******* " + balanceNeeded, this.programId, this.publicKey, newAccount.publicKey, owner)
    transaction.add(
  ...
  async createPortfolioAccount(owner: PublicKey, accountSize: any): Promise<PublicKey> {
    // Allocate memory for the account
    const balanceNeeded = await Token.getMinBalanceRentForExemptAccount(
      this.connection//,accountSize
    );
  ...
  async getPortfolioAccountInfo(
    ...
    //@ts-ignore
    const data = Buffer.from(info.data);
    const accountInfo = PortfolioAccountLayout.decode(data);
    accountInfo.owner = new PublicKey(accountInfo.owner);
    console.log("**accountInfo*** " + JSON.stringify(accountInfo))
    return accountInfo;
  }
  ...
  async createPortfolio(connection: any,newPortfolio:any, selectedWallet: any, portfolio: any): Promise<PublicKey> {
    console.log("start");
    ...
    let testAccount: any = await ntoken.createPortfolio(owner, metaDataUrl, metaDataHash,
