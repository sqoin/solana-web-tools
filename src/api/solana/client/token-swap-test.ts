...
export async function createTokenSwap(connection:Connection): Promise<void> {
  ...
  let tokenPool = await TokenOrigin.createMint(
      connection,
      payer,
      authority,
      null,
  ...
  mintA = new Token(
    ...
    new PublicKey(ORIGINE_PROGRAMM_ID),//TOKEN_PROGRAM_ID,
    payer
  )
...
export async function swap(connection: any, selectedWallet: any): Promise<void> {
  ...
  info = await mintA.getAccountInfo(userAccountA);
  console.log("info after swap of userAccountA mintToken A-" + info.mint + " -owner -" + info.owner + " - amount - " + info.amount)

  info = await mintB.getAccountInfo(userAccountB);
  console.log("info after swap of userAccountB mintToken B-" + info.mint + " -owner -" + info.owner + " - amount - " + info.amount)

  info = await mintA.getAccountInfo(tokenAccountA);
  console.log("info after swap of TokenAccountA mintToken A-" + info.mint + " -owner -" + info.owner + " - amount - " + info.amount)

  currentSwapTokenA += SWAP_AMOUNT_IN;

  info = await mintB.getAccountInfo(tokenAccountB);
  console.log("info after swap of TokenAccountB mintToken B-" + info.mint + " -owner -" + info.owner + " - amount - " + info.amount)
  /* 
    assert(info.amount.toNumber() == currentSwapTokenB - SWAP_AMOUNT_OUT);
    currentSwapTokenB -= SWAP_AMOUNT_OUT; */

  info = await tokenPool.getAccountInfo(tokenAccountPool);
  console.log("amount token pool " + info.amount.toNumber() + " - publickKeyPool - " + info.mint)
